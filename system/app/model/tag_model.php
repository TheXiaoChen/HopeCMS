...
class Tag_Model {
    ...
    function __construct() {
        ...
        $this->table_blog = DB_PREFIX . 'article';
    }
    ...
    function updateTagName($tagId, $tagName, $title, $keywords, $description)
    {
        $sql = "UPDATE $this->table SET tagname='$tagName', keywords='$keywords', title='$title', description='$description' WHERE tid=$tagId";
        $this->db->query($sql);
    }
    ...
    function getTagsCount($tag_name = '') {
        $condition = '';
        if ($tag_name) {
            $tag_name = $this->db->escape_string($tag_name);
            $condition = " WHERE tagname LIKE '%{$tag_name}%'";
        }
        $data = $this->db->once_fetch_array("SELECT COUNT(*) AS total FROM $this->table{$condition}");
