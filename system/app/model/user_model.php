...
class User_Model {
    ...
    public function getUsers($email = '', $nickname = '', $order = '', $page = 1, $perpage_num = 15)
    {
        $condition = $limit = '';
        if ($email) {
    ...
    public function updateUser($userData, $uid) {
        ...
        foreach ($userData as $key => $data) {
            ...
            $sets[] = "`{$key}`='" . $this->db->escape_string((string)$data) . "'";
        }
        if (empty($sets)) {
            return;
        }
        $this->db->query('UPDATE ' . $this->table . ' SET ' . implode(',', $sets) . ' WHERE uid=' . $uid);
    }
    ...
    public function deleteUser($uid) {
        $this->db->query("update " . DB_PREFIX . "article set author=1, checked='y' where author=$uid");
