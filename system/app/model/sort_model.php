<?php
/**
 * Hope CMS(希望CMS) article sort model
...
class Sort_Model {
    ...
    function getSorts() {
        ...
        while ($row = $this->db->fetch_array($query)) {
            $data = $this->db->once_fetch_array("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "article WHERE sortid=" . $row['sid'] . " AND hide='n' AND checked='y' AND type='blog'");
            $logNum = $data['total'];
            $sortData = array(
                'lognum'      => $logNum,
                ...
                'logstyle'    => htmlspecialchars($row['logstyle']),
                // ResHub 等主题读取 template；与列表模板 style 对齐
                'template'    => htmlspecialchars(trim((string)($row['style'] ?? ''))),
