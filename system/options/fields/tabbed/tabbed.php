...
if ( ! class_exists( 'CSF_Field_tabbed' ) ) {
  class CSF_Field_tabbed extends CSF_Fields {
    ...
    public function render() {
      ...
      echo '<div class="hope-tabbed-nav">';
      foreach ( $this->field['tabs'] as $key => $tab ) {

        $tabbed_icon   = ( ! empty( $tab['icon'] ) ) ? '<i class="hope--icon '. esc_attr( $tab['icon'] ) .'"></i>' : '';
        $tabbed_active = ( empty( $key ) ) ? 'hope-tabbed-active' : '';
      ...
      echo '<div class="hope-tabbed-contents">';
      foreach ( $this->field['tabs'] as $key => $tab ) {

        $tabbed_hidden = ( ! empty( $key ) ) ? ' hidden' : '';

        echo '<div class="hope-tabbed-content'. esc_attr( $tabbed_hidden ) .'">';
