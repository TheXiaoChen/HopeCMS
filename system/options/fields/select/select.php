  ...
  class CSF_Field_select extends CSF_Fields
  {
    ...
    public function render()
    {
      ...
      if (isset($this->field['options'])) {
        ...
        $chosen_sortable  = ($args['chosen'] && $args['sortable']) ? ' hope-chosen-sortable' : '';
        $chosen_ajax      = ($args['chosen'] && $args['ajax']) ? ' hope-chosen-ajax' : '';
        $placeholder_attr = ($args['chosen'] && $args['placeholder']) ? ' data-placeholder="' . esc_attr($args['placeholder']) . '"' : '';
        $field_class      = ($args['chosen']) ? ' class="hope-chosen' . esc_attr($chosen_sortable . $chosen_ajax) . '"' : '';
        ...
        if (is_string($maybe_options) && !empty($args['chosen']) && !empty($args['ajax'])) {
          $options = $this->field_query_data_title($maybe_options, $this->value);
        ...
        if ((is_array($options) && !empty($options)) || (!empty($args['chosen']) && !empty($args['ajax']))) {
          ...
          if (!empty($args['chosen']) && !empty($args['multiple'])) {
            ...
            echo '<select name="' . $field_name . '" class="hope-hide-select hidden"' . $multiple_attr . $field_attr . '>';
