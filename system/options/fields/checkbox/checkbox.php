...
if (!class_exists('CSF_Field_checkbox')) {
    class CSF_Field_checkbox extends CSF_Fields
    {
        ...
        public function render()
        {
            ...
            $inline_class = ($args['inline']) ? ' class="hope--inline-list"' : '';
            ...
            if (isset($this->field['options'])) {
                    ...
                    foreach ($options as $option_key => $option_value) {
                        ...
                        if (is_array($option_value) && !empty($option_value)) {
                            ...
                            foreach ($option_value as $sub_key => $sub_value) {
                                ...
                                echo '<span class="hope--text">' . esc_attr($sub_value) . '</span>';
                                echo '</label>';
                                echo '</li>';
                            }
                            echo '</ul>';
                            ...
                            echo '<span class="hope--text">' . esc_attr($option_value) . '</span>';
                            echo '</label>';
                            echo '</li>';
                        }
                    }
                ...
                echo '<label class="hope-checkbox">';
                echo '<input type="hidden" name="' . esc_attr($this->field_name()) . '" value="' . $this->value . '" class="hope--input"' . $this->field_attributes() . '/>';
                echo '<input type="checkbox" name="_pseudo" class="hope--checkbox"' . esc_attr(checked($this->value, 1, false)) . '/>';
                echo (!empty($this->field['label'])) ? '<span class="hope--text">' . esc_attr($this->field['label']) . '</span>' : '';
                echo '</label>';
            }
