...
if ( ! class_exists( 'CSF_Field_dimensions' ) ) {
  class CSF_Field_dimensions extends CSF_Fields {
    ...
    public function render() {
      ...
      $is_unit = ( ! empty( $unit ) ) ? ' hope--is-unit' : '';
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="hope--label hope--icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. $placeholder .' class="hope-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="hope--label hope--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="hope--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="hope--label hope--icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. $placeholder .' class="hope-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="hope--label hope--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
