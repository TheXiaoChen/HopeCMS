...
class Mysqlpdo {
    ...
    private function __construct() {
        ...
        try {
            ...
            hpMsg("连接数据库失败，请检查数据库信息。错误原因：" . $e->getMessage());
        }
    ...
    function query($sql, $ignore_err = FALSE) {
        ...
        try {
            ...
            if (!$ignore_err && !$this->result) {
                hpMsg("SQL语句执行错误: {$sql}<br />" . $this->geterror());
            } else {
                return $this->result;
            }
        } catch (\PDOException $e) {
            hpMsg("SQL语句执行错误: {$sql}<br />" . $e->getMessage());
        }
    ...
    function escape_string($sql) {
        if (is_null($this->conn)) {
            return addslashes(trim($sql));
        }
        $quoted = $this->conn->quote(trim($sql));
        // PDO::quote 会添加引号，需要去掉
