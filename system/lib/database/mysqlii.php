<?php
/**
 * Hope CMS(希望CMS) MySQLi Database Class
...
class MySqlii {
    ...
    private function __construct() {
        ...
        if ($this->conn->connect_error) {
            switch ($this->conn->connect_errno) {
                case 1044:
                case 1045:
                    hpMsg("连接MySQL数据库失败，数据库用户名或密码错误");
                    break;
                case 1049:
                    hpMsg("连接MySQL数据库失败，未找到你填写的数据库");
                    break;
                case 2003:
                case 2005:
                case 2006:
                    hpMsg("连接MySQL数据库失败，数据库地址错误或者数据库服务器不可用");
                    break;
                default :
                    hpMsg("连接MySQL数据库失败，请检查数据库信息。错误信息：" . $this->conn->connect_error);
                    break;
            }
        }
    ...
    public function query($sql, $ignore_err = FALSE) {
        ...
        if (!$ignore_err && 1046 == $this->getErrNo()) {
            hpMsg("连接数据库失败，请填写数据库名");
        }
        if (!$ignore_err && 1115 == $this->getErrNo()) {
            hpMsg("MySQL缺少utf8mb4字符集，请升级到MySQL5.6或更高版本");
        }
        if (!$ignore_err && !$this->result) {
            hpMsg("$sql<br /><br />error: " . $this->getErrNo() . ' , ' . $this->getError());
        } else {
            return $this->result;
        }
    }
