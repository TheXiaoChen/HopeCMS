...
class Calendar {
    ...
    static function generate() {
        ...
        $query = $DB->query("SELECT date FROM " . DB_PREFIX . "article WHERE hide='n' and checked='y' and type='blog' $date_state");
        ...
        // 年月规范化，避免 "08" 等字符串运算异常
        $n_year = (int)$n_year;
        $n_year2 = (int)$n_year2;
        $n_month = str_pad((string)((int)$n_month), 2, '0', STR_PAD_LEFT);
        if ((int)$n_month < 1 || (int)$n_month > 12) {
            $n_month = date('m');
            $n_year = (int)date('Y');
            $n_year2 = $n_year;
        }
        $year_month = sprintf('%04d%02d', $n_year, (int)$n_month);
        ...
        $mj = str_pad((string)$mj, 2, '0', STR_PAD_LEFT);
        ...
        <td><a href=\"javascript:void(0);\" onclick=\"sendinfo('" . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . "');\"> &laquo; </a>$n_year2<a href=\"javascript:void(0);\" onclick=\"sendinfo('" . htmlspecialchars($url2, ENT_QUOTES, 'UTF-8') . "');\"> &raquo; </a></td>
        <td><a href=\"javascript:void(0);\" onclick=\"sendinfo('" . htmlspecialchars($url3, ENT_QUOTES, 'UTF-8') . "');\"> &laquo; </a>$n_month<a href=\"javascript:void(0);\" onclick=\"sendinfo('" . htmlspecialchars($url4, ENT_QUOTES, 'UTF-8') . "');\"> &raquo; </a></td>
        </tr></table>
        ...
        //获取给定年月的第一天是星期几
        $week = (int)@gmdate("w", gmmktime(0, 0, 0, (int)$n_month, 1, $n_year));
        //获取给定年月的天数
        $lastday = (int)@gmdate("t", gmmktime(0, 0, 0, (int)$n_month, 1, $n_year));
        //获取给定年月的最后一天是星期几
        $lastweek = (int)@gmdate("w", gmmktime(0, 0, 0, (int)$n_month, $lastday, $n_year));
        if ($week == 0) {
            $week = 7;
        }
        $j = 1;
